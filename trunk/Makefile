...
image: boot/512.bin kernel/gdt.bin kernel/kernel.bin video/video.bin shell/shell.bin Makefile
	...
	dd if=shell/shell.bin of=image count=$$(( ($(_SHELL_SIZE) / $(SECTOR_SIZE)) )) seek=$$(( $(OFFSET) + ( $(_GDT_SIZE) + $(_KERNEL_SIZE) + $(_VIDEO_SIZE)) / $(SECTOR_SIZE)  ))
...
clean:
	cd boot && make clean && cd ../kernel && make clean && cd ../video && make clean && cd ../shell && make clean && cd ../usrlib && make clean
